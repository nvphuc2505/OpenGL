...
void multiply_matrix_to_vector(float matrix[16], vec4 &vec)
{
    float tmp[4];
    for(int i = 0; i < 4; i++)
    {
    ...
    vec.v[0] = matrix[0] * tmp[0] + matrix[1] * tmp[1] + matrix[2] * tmp[2] + matrix[3] * tmp[3];
    vec.v[1] = matrix[4] * tmp[0] + matrix[5] * tmp[1] + matrix[6] * tmp[2] + matrix[7] * tmp[3];
    vec.v[2] = matrix[8] * tmp[0] + matrix[9] * tmp[1] + matrix[10] * tmp[2] + matrix[11] * tmp[3];
    vec.v[3] = matrix[12] * tmp[0] + matrix[13] * tmp[1] + matrix[14] * tmp[2] + matrix[15] * tmp[3];
}